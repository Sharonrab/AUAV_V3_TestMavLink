...
HilMessageType hil_message_order[] = {HIL_GPS, HIL_GPS_DATE_TIME, HIL_AIR, HIL_RAW, HIL_RAW_AIR};
...
boost::tuple<uint8_t*, size_t> SlugsMavlinkParser::parse_udp2serial(uint8_t *buf, size_t bytes) {
	...
	// Return message and length as tuple
...
size_t SlugsMavlinkParser::assemble_mavlink_message(uint8_t* rawUdpData, uint8_t* buf, HilMessageType type) { 
	...
	switch (type) {
		case HIL_GPS:
		{
			...
			mlGpsData.fix_type = 3;
			mlGpsData.satellites_visible = 99;
#ifdef DEBUG
		...
		case HIL_ATTITUDE:
		{
			...
			//SR
			mlAttitudeData.time_boot_ms = mlAttitude.time_boot_ms;
			// Encode MAVLink message
			#ifdef DEBUG
