/*
 * -------------------------------------------------------------------
 * MPLAB 16-Bit Device Blocks for Simulink v3.38.
 *
 *   Product Page:  http://www.microchip.com/SimulinkBlocks
 *           Forum: http://www.microchip.com/forums/f192.aspx
 *           Wiki:  http://microchip.wikidot.com/simulink:start
 * -------------------------------------------------------------------
 * File: rt_sys_SLUGS2_77.h
 *
 * Code generated for Simulink model 'SLUGS2'.
 *
 * Model version                  : 1.271
 * Simulink Coder version         : 8.8 (R2015a) 09-Feb-2015
 * C/C++ source code generated on : Tue Apr 11 15:47:53 2017
 */

#ifndef RTW_HEADER_rt_sys_SLUGS2_77_h_
#define RTW_HEADER_rt_sys_SLUGS2_77_h_
#ifndef SLUGS2_COMMON_INCLUDES_
# define SLUGS2_COMMON_INCLUDES_
#include "rtwtypes.h"
#endif                                 /* SLUGS2_COMMON_INCLUDES_ */

#include "SLUGS2_types.h"

/* Shared type includes */
#include "multiword_types.h"

/* Block signals for system '<S471>/Embedded MATLAB Function' */
typedef struct {
  real32_T y;                          /* '<S471>/Embedded MATLAB Function' */
} rtB_EmbeddedMATLABFunction_d_T;

extern void S_EmbeddedMATLABFunction_l(real32_T rtu_u,
  rtB_EmbeddedMATLABFunction_d_T *localB);

#endif                                 /* RTW_HEADER_rt_sys_SLUGS2_77_h_ */

/*
 * File trailer for generated code.
 *
 * [EOF]
 */
